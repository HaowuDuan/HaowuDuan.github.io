...
\begin{enumerate}
    ...
    with the operator \(\widehat{a}_t\).  Then
\end{enumerate}

\begin{equation}
    \begin{split}
    \widehat{\pi}|S_t\rangle
    &=\int da_t\;\pi(a_t\mid S_t)\widehat{a}_t|S_t\rangle\\
    &\equiv\widehat{\pi}(S_t)|S_t\rangle,
    \end{split}
    \label{eq:policy-operator}
\end{equation}

where
\(\widehat{\pi}(S_t)=\int da_t\,\pi(a_t\mid S_t)\widehat{a}_t\).
