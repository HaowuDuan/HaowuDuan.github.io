...
if (!texPath || !outputDirectory) {
  throw new Error('Usage: node render-optimal-transport-figure.mjs <tex> <output-directory>');
}
...
const figures = [...source.matchAll(/\\begin\{figure\}(?:\[[^\]]*\])?([\s\S]*?)\\end\{figure\}/g)]
  .map(match => {
    ...
    if (!label || !tikz) {
      throw new Error('Every optimal-transport figure must have one label and one TikZ picture');
    }
    return { label, tikz };
  });

if (figures.length !== 1) {
  throw new Error(`Expected 1 optimal-transport figure, found ${figures.length}`);
}

const temporaryDirectory = await mkdtemp(join(tmpdir(), 'optimal-transport-figure-'));
...
process.stdout.write(`Rendered ${figures.length} optimal-transport figure to ${outputDirectory}.\n`);
