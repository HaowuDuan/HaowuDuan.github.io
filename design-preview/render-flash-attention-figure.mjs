...
if (!texPath || !outputDirectory) {
  throw new Error('Usage: node render-flash-attention-figure.mjs <tex> <output-directory>');
}
...
const figures = [...source.matchAll(/\\begin\{figure\}(?:\[[^\]]*\])?([\s\S]*?)\\end\{figure\}/g)]
  .map(match => {
    ...
    if (!label || !tikz) {
      throw new Error('Every Flash Attention figure must have one label and one TikZ picture');
    }
    return { label, tikz };
  });

if (figures.length !== 1) {
  throw new Error(`Expected 1 Flash Attention figure, found ${figures.length}`);
}

const temporaryDirectory = await mkdtemp(join(tmpdir(), 'flash-attention-figure-'));
...
process.stdout.write(`Rendered ${figures.length} Flash Attention figure to ${outputDirectory}.\n`);
