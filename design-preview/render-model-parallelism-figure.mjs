...
if (!texPath || !outputDirectory) {
  throw new Error('Usage: node render-model-parallelism-figure.mjs <tex> <output-directory>');
}
...
const figures = [...source.matchAll(/\\begin\{figure\}(?:\[[^\]]*\])?([\s\S]*?)\\end\{figure\}/g)]
  .map(match => {
    ...
    if (!label || !tikz) {
      throw new Error('Every model-parallelism figure must have one label and one TikZ picture');
    }
    return { label, tikz };
  });

if (figures.length !== 1) {
  throw new Error(`Expected 1 model-parallelism figure, found ${figures.length}`);
}

const temporaryDirectory = await mkdtemp(join(tmpdir(), 'model-parallelism-figure-'));
...
process.stdout.write(`Rendered ${figures.length} model-parallelism figure to ${outputDirectory}.\n`);
