...
if (!htmlPath || !texPath) {
  throw new Error('Usage: node prepare-optimal-transport-note.mjs <html> <tex>');
}
...
let html = sourceHtml.replace(
  ...
  (_, citedKeys) => {
    const links = citedKeys.split(/\s+/).map(key => {
      ...
      if (!number) throw new Error(`Unknown optimal-transport bibliography key: ${key}`);
...
const figureLabel = 'fig:schrodinger-reweighting';
...
html = html.replace(
  figurePattern,
  '$1\n<img src="assets/optimal-transport/schrodinger-reweighting.svg" alt="Reference trajectories with a central terminal mode are reweighted into bridge trajectories with two outer terminal modes." />\n',
);
...
if (!bibliographyMatch) {
  throw new Error('Pandoc did not emit the optimal-transport bibliography');
}
...
if (bibliographyParagraphs.length !== allBibliographyKeys.length) {
  throw new Error(
    `Optimal-transport bibliography mismatch: ${bibliographyParagraphs.length} entries for ${allBibliographyKeys.length} keys`,
  );
}
...
process.stdout.write(
  `${htmlPath}: linked ${bibliographyKeys.length} references and mapped the Schrödinger-bridge figure\n`,
);
